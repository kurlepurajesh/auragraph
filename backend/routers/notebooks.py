...
async def save_notebook_note(
    ...
    async with deps._db_write_lock:
        return update_notebook_note(
            nb_id,
            req.note,
            req.proficiency,
            save_version=not bool(req.skip_version),
            discard_matching_version=bool(req.discard_matching_version),
        )


@router.get("/api/notebooks/{nb_id}/note-versions")
async def get_note_versions(
    nb_id: str,
    authorization: Optional[str] = Header(None),
    limit: int = Query(default=5, ge=1, le=20),
):
    from agents.notebook_store import list_note_versions
    user = get_current_user(authorization)
    _require_notebook_owner(nb_id, user)
    return {"versions": list_note_versions(nb_id, limit=limit)}


@router.post("/api/notebooks/{nb_id}/note-versions/{version_id}/restore")
async def restore_note_version_endpoint(
    ...
    restored = restore_note_version(nb_id, version_id, proficiency=nb.get("proficiency"))
    if not restored:
...
@router.patch("/api/notebooks/{nb_id}/pages/{page_idx}")
async def save_notebook_page(
    nb_id: str,
    page_idx: int,
    content: str = Body(..., embed=True),
    authorization: Optional[str] = Header(None),
):
    """Update one note page efficiently and persist to backend page store."""
    from agents.knowledge_store import get_all_note_pages, store_note_pages
    from agents.notebook_store import update_notebook_note

    if page_idx < 0:
        raise HTTPException(400, "page_idx must be >= 0")
    if not isinstance(content, str) or not content.strip():
        raise HTTPException(400, "Page content cannot be empty.")
    if len(content) > 200_000:
        raise HTTPException(413, "Page content is too large.")
    ...
    async with deps._db_write_lock:
        ...
        if not pages:
            raw_note = (nb.get("note") or "").strip()
            if raw_note:
                pages = [p.strip() for p in re.split(r'(?m)^(?=## )', raw_note) if p.strip()]
                if pages:
                    store_note_pages(nb_id, pages)
        ...
        store_note_pages(nb_id, pages)
        updated_nb = update_notebook_note(nb_id, "\n\n".join(pages))

    return {
        "ok": True,
        "page_idx": page_idx,
        "page": content,
        "note": (updated_nb or {}).get("note", ""),
    }
