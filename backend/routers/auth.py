...
class EducationProfileRequest(BaseModel):
    class_level: str = Field(default="", max_length=80)
    education_stage: str = Field(default="", max_length=80)
    institution: str = Field(default="", max_length=120)
    board_university: str = Field(default="", max_length=120)
    stream_major: str = Field(default="", max_length=120)
    target_exam: str = Field(default="", max_length=120)
...
async def get_auth_profile(authorization: Optional[str] = Header(None)):
    from agents.auth_utils import get_user_profile
    user = get_current_user(authorization)
    return {"profile": get_user_profile(user["id"])}


@router.put("/auth/profile")
async def update_auth_profile(
    req: EducationProfileRequest,
    authorization: Optional[str] = Header(None),
):
    from agents.auth_utils import update_user_profile
    user = get_current_user(authorization)
    profile = update_user_profile(user["id"], req.model_dump())
