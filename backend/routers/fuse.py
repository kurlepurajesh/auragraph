...
def _sanitize_text_light(text: str) -> str:
    """Remove minor profanity while preserving educational meaning."""
    if not text:
        return text or ""
    try:
        from agents.content_safety import sanitise_input
...
def _normalize_custom_note_instruction(text: Optional[str]) -> str:
    cleaned = _sanitize_text_light(text or "").strip()
    return cleaned[:600] if cleaned else ""


def _looks_educational(text: str) -> bool:
    """Fast heuristic to reject clearly irrelevant/non-educational payloads."""
    if not text or not text.strip():
        return False

    sample = text[:20_000]
    words = re.findall(r"[a-zA-Z]{3,}", sample.lower())
    if len(words) < 24:
        return False

    indicators = {
        "lecture", "chapter", "unit", "topic", "concept", "definition", "theorem",
        "proof", "equation", "formula", "derive", "derivation", "algorithm", "method",
        "example", "exercise", "problem", "analysis", "model", "matrix", "vector",
        "probability", "statistics", "signal", "system", "calculus", "physics", "chemistry",
        "biology", "economics", "engineering", "computer", "programming", "architecture",
        "data", "function", "integral", "differential",
    }
    uniq = set(words)
    keyword_hits = len(uniq & indicators)

    math_markers = 0
    for m in ("=", "=>", "->", "sum", "integral", "\n##", "--- Slide", "--- Page"):
        if m in sample:
            math_markers += 1

    return keyword_hits >= 2 or (keyword_hits >= 1 and math_markers >= 1) or math_markers >= 3


async def _safety_and_relevance_guard(slides_text: str, textbook_text: str, source_hint: str = "uploaded content") -> tuple[str, str]:
    """
    Reuse existing content safety + fast heuristics to prevent junk-note generation.
    Returns sanitised (slides_text, textbook_text) or raises HTTPException.
    """
    from agents.content_safety import check_input

    slides_clean = _sanitize_text_light(slides_text or "")
    textbook_clean = _sanitize_text_light(textbook_text or "")
    combined = (slides_clean + "\n\n" + textbook_clean).strip()

    if not combined:
        raise HTTPException(
            422,
            "No notes could be generated from this upload. Please provide clear educational content (lecture notes/slides/transcript).",
        )

    # Existing AI content safety: block only severe harmful payloads.
    safe, cat = await check_input(combined[:6000])
    if not safe:
        raise HTTPException(
            422,
            f"No notes could be generated from this {source_hint}. Please provide safe educational content.",
        )

    # Fast local relevance guard (no extra network latency).
    if not _looks_educational(combined):
        raise HTTPException(
            422,
            "No notes could be generated. Please upload educational content related to a subject/topic.",
        )

    return slides_clean, textbook_clean


def _safe_image_ext(mime: str) -> str:
    raw = (mime or "image/png").split("/")[-1].lower().replace("jpeg", "jpg")
    raw = raw.split(";")[0].strip()
    ext = re.sub(r"[^a-z0-9]", "", raw)
    return ext or "png"


def _image_filename(img) -> str:
    return f"{img.img_id}.{_safe_image_ext(getattr(img, 'mime', 'image/png'))}"


def _image_url_if_exists(notebook_id: str, img) -> str:
    if not notebook_id:
        return ""
    from agents.slide_images import get_image_path
    fname = _image_filename(img)
    path = get_image_path(notebook_id, fname)
    return f"/api/images/{notebook_id}/{fname}" if path else ""
...
async def upload_fuse_multi(
    ...
    custom_note_instruction: Optional[str] = Form(None),
    media_transcript_chunks: Optional[str] = Form(None),
    ...
    custom_note_instruction = _normalize_custom_note_instruction(custom_note_instruction)
    if notebook_id:
    ...
    all_slides_text, all_textbook_text = await _safety_and_relevance_guard(
        all_slides_text,
        all_textbook_text,
        source_hint="uploaded files",
    )
    ...
    if all_textbook_images and notebook_id:
        ...
        try:
            ...
            for img in all_textbook_images:
                img_url = _image_url_if_exists(notebook_id, img)
                if img_url:
                    textbook_figures_items.append((img, img_url))
        except Exception:
            all_textbook_images = []
    ...
    if topics:
        try:
            ...
            try:
                ...
                from agents.auth_utils import get_user_profile as _get_uprof, profile_to_student_context as _prof_ctx
                _uid = user.get("id", "") if isinstance(user, dict) else ""
                _beh_ctx = await _aio_f.to_thread(_get_pctx, _uid)
                _edu_profile = await _aio_f.to_thread(_get_uprof, _uid)
                _edu_ctx = _prof_ctx(_edu_profile, proficiency=proficiency)
                _notes_feedback = _feedback_guidance("notes")
                _stud_ctx = _merge_student_contexts(_beh_ctx, _edu_ctx, _notes_feedback)
            except Exception:
                _stud_ctx = ""
            fused_note, source = await asyncio.wait_for(
                run_generation_pipeline(topics=topics, topic_contexts=topic_contexts,
                                        ...
                                        custom_note_instruction=custom_note_instruction),
    ...
    if not fused_note or len(fused_note.strip()) < 100:
        fused_note = generate_local_note(
            all_slides_text,
            all_textbook_text,
            proficiency,
            custom_note_instruction=custom_note_instruction,
...
async def upload_fuse(
    ...
    return await upload_fuse_multi(
        ...
        proficiency=proficiency, custom_note_instruction=custom_note_instruction,
...
async def upload_fuse_stream(
    ...
    async def event_generator():
        ...
        try:
            ...
            from agents.auth_utils import get_user_profile as _get_prof_s, profile_to_student_context as _prof_ctx_s
            _beh_ctx_s = await _aio_s.to_thread(_gpctx_s, user_id_for_stream)
            _edu_profile_s = await _aio_s.to_thread(_get_prof_s, user_id_for_stream)
            _edu_ctx_s = _prof_ctx_s(_edu_profile_s, proficiency=proficiency)
            _notes_feedback = _feedback_guidance("notes")
            _stream_student_ctx = _merge_student_contexts(_beh_ctx_s, _edu_ctx_s, _notes_feedback)
        except Exception:
            pass

        final_note, final_source = "", "local"
        try:
            async for event in run_generation_pipeline_stream(
                topics,
                topic_contexts,
                proficiency,
                student_context=_stream_student_ctx,
                custom_note_instruction=custom_note_instruction,
            ):
            ...
            if not final_note:
                final_note = generate_local_note(
                    all_slides_text,
                    all_textbook_text,
                    proficiency,
                    custom_note_instruction=custom_note_instruction,
...
async def fuse_knowledge(req: FusionRequest, authorization: Optional[str] = Header(None)):
    ...
    slide_content, textbook_content = await _safety_and_relevance_guard(
        slide_content,
        textbook_content,
        source_hint="content",
    )
    nb_id            = req.notebook_id
    custom_note_instruction = _normalize_custom_note_instruction(req.custom_note_instruction)
    ...
    try:
        ...
        if topics:
            ...
            _legacy_student_ctx = ""
            try:
                from agents.behaviour_store import get_personalisation_context as _gpctx_legacy
                from agents.auth_utils import get_user_profile as _gprof_legacy, profile_to_student_context as _prof_ctx_legacy
                import asyncio as _aio_legacy
                _beh_ctx_legacy = await _aio_legacy.to_thread(_gpctx_legacy, user["id"])
                _profile_legacy = await _aio_legacy.to_thread(_gprof_legacy, user["id"])
                _edu_ctx_legacy = _prof_ctx_legacy(_profile_legacy, proficiency=req.proficiency)
                _notes_feedback = _feedback_guidance("notes")
                _legacy_student_ctx = _merge_student_contexts(_beh_ctx_legacy, _edu_ctx_legacy, _notes_feedback)
            except Exception:
                pass
            fused_note, source = await run_generation_pipeline(
                ...
                student_context=_legacy_student_ctx,
                custom_note_instruction=custom_note_instruction,
            )
    except Exception as exc:
...
async def media_ingest(
    ...
    # Sanitize small foul words but reject clearly harmful/irrelevant media payloads.
    chunks_clean = [_sanitize_text_light(c) for c in all_chunks if c and c.strip()]
    combined_media = "\n\n".join(chunks_clean)
    try:
        combined_media, _ = await _safety_and_relevance_guard(
            combined_media,
            "",
        ...
        return MediaIngestResponse(
            ...
            chunks=[],
            source_desc="",
            message="No notes could be generated from this media. Please provide clear educational content.",
        )

    all_chunks = chunks_clean
