...
from deps import get_current_user, _is_azure_available, _is_groq_available, _azure_chat, _groq_chat
...
async def _ai_transcript_for_tts(text: str, voice_key: str) -> str:
    """
    Build a speech-friendly transcript using a small/faster LLM prompt.
    Falls back to deterministic cleaning on any model failure.
    """
    source = _html.unescape(_clean_text_for_tts(text))
    if not source:
        return source

    voice = VOICES.get(voice_key, VOICES[DEFAULT_VOICE])
    lang = voice.get("lang", "en-IN")

    system = (
        "You convert study notes into a clean read-aloud transcript for text-to-speech. "
        "Keep content faithful and complete, but remove markup and notation that sounds unnatural when spoken."
    )
    user = (
        "Create a plain-text speech transcript in the same language as the input "
        f"(target voice language hint: {lang}).\n\n"
        "Strict rules:\n"
        "1) Keep all conceptual content and key terms; do not drop meaning.\n"
        "2) Remove markdown syntax (#, *, >, backticks, tables, etc.).\n"
        "3) Convert mathematical notation to natural spoken math, not symbols.\n"
        "   Example: x^2 -> x squared, integral_0^1 f(x) dx -> integral from zero to one of f of x d x.\n"
        "4) Do not say words like 'latex', 'formula symbol', 'dollar sign'.\n"
        "5) Output plain text only, no bullets, no labels, no JSON.\n\n"
        "Input:\n"
        f"{source}"
    )
    ...
    try:
        ...
        if _is_groq_available():
            out = await _groq_chat(messages, max_tokens=2200)
        elif _is_azure_available():
            out = await _azure_chat(messages, max_tokens=2200)
        else:
            return source
        ...
        # Guard: avoid model responses that are suspiciously short.
        if len(out) < max(120, int(len(source) * 0.30)):
            return source

        return out
    except Exception as exc:
        logger.warning("TTS transcript rewrite failed; using deterministic cleaner: %s", exc)
        return source
...
async def synthesize_speech(
    ...
    transcript = await _ai_transcript_for_tts(req.text, req.voice)
