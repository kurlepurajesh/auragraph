...
_PROFICIENCY_COMPACT = """PROFICIENCY OVERRIDE: COMPACT REVISION NOTES

The user explicitly requested short/concise notes.

Apply these compact-mode rules:
1) Keep FULL TOPIC COVERAGE (do not skip slide concepts), but use MINIMAL depth.
2) Prefer concise bullets and short paragraphs.
3) No long derivations, no lengthy analogies, no verbose explanations.
4) Definitions should be one-line max, and only when essential to understand formula usage.
5) Focus on: key formulas, key conditions, high-yield exam points, and pitfalls.
6) Keep each ### sub-section brief and scannable.

When compact mode conflicts with normal proficiency length/depth targets,
COMPACT MODE takes precedence.
"""


def _wants_compact_notes(custom_note_instruction: str) -> bool:
    text = (custom_note_instruction or "").strip().lower()
    if not text:
        return False
    compact_signals = (
        "shortnotes", "short notes", "concise", "brief", "crisp",
        "summary", "revision notes", "quick notes", "no definitions",
        "without definitions", "skip definitions",
    )
    return any(sig in text for sig in compact_signals)


def _effective_proficiency_block(proficiency: str, custom_note_instruction: str) -> str:
    if _wants_compact_notes(custom_note_instruction):
        return _PROFICIENCY_COMPACT
    return _resolve_proficiency_block(proficiency)
...
def _user_instruction_block(custom_note_instruction: str) -> str:
    """Build a safe optional style-guidance block from user preferences."""
    text = (custom_note_instruction or "").strip()
    if not text:
        return ""
    text = text[:600]
    return (
        "USER PREFERENCE (HIGH PRIORITY STYLE OVERRIDE):\n"
        f"{text}\n\n"
        "PRIORITY RULE: Respect this preference for style, density, and depth."
        " If it asks for concise notes, override normal verbosity targets while keeping"
        " core topic coverage intact."
    )
...
async def _generate_subchunk(
    ...
    user = _safe_format(
        ...
        proficiency_block=_effective_proficiency_block(proficiency, custom_note_instruction),
        custom_instruction_block=_user_instruction_block(custom_note_instruction),
    )
...
async def run_generation_pipeline(
    ...
    async def _generate_with_sem(topic: SlideTopic) -> tuple[str, str]:
        ...
        return await generate_topic_note(
            topic,
            context,
            proficiency,
            api_sem=_api_sem,
...
async def run_generation_pipeline_stream(
    ...
    async def _wrapped(t: SlideTopic, idx: int):
        ...
        section, src = await generate_topic_note(
            t,
            ctx,
            proficiency,
            api_sem=api_sem,
