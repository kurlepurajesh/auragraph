...
════════════════════════════════
TASK:
1. Diagnose the student's conceptual gap in crisp and short sentences.
2. Produce the ENHANCED NOTE PAGE following these ABSOLUTE RULES:
   ...
   ╚══════════════════════════════════════════════════════════════╝
    ...
    Placement policy (MANDATORY):
    - Insert new content as close as possible to the line/paragraph that triggered the doubt.
    - If the doubt appears to come from highlighted text, place additions immediately after that exact nearby passage.
    - If the doubt is general, place additions in the most semantically relevant existing subsection.
    - NEVER prepend additions at the very top of the page or before the first heading.
    - Do not dump all additions in one block; distribute them near the relevant parts of the note.

    What to ADD (following the placement policy above):
