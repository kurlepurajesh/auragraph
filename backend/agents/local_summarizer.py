...
def _wants_shortnotes(custom_note_instruction: str) -> bool:
    text = (custom_note_instruction or "").strip().lower()
    if not text:
        return False
    return any(sig in text for sig in (
        "shortnotes", "short notes", "concise", "brief", "crisp",
        "quick notes", "revision notes", "summary",
    ))


def _wants_no_definitions(custom_note_instruction: str) -> bool:
    text = (custom_note_instruction or "").strip().lower()
    if not text:
        return False
    return any(sig in text for sig in (
        "no definitions", "without definitions", "skip definitions",
        "no need definitions", "no need the definitions",
    ))


def _apply_user_style_overrides(note: str, custom_note_instruction: str) -> str:
    if not note or not custom_note_instruction:
        return note

    out = note
    if _wants_no_definitions(custom_note_instruction):
        out = re.sub(
            r'(?ms)^###\s*(?:📖\s*)?(?:Definition|What\s+Is\s+It\?|Formal\s+Definition)\s*\n.*?(?=^###\s|^##\s|\Z)',
            '',
            out,
        )
    out = re.sub(r'\n{3,}', '\n\n', out).strip()
...
def generate_local_note(
    ...
    """
    compact_mode = _wants_shortnotes(custom_note_instruction)
    effective_proficiency = "Practitioner" if compact_mode else proficiency

    cfg     = _PROF.get(effective_proficiency, _PROF["Practitioner"])
    prose_k = cfg["prose_k"]
    if compact_mode:
        prose_k = min(prose_k, 4)
        ...
        if textbook_paragraphs:
            ...
            if best_para:
                enrichment = _extract_enrichment(
                    best_para,
                    max_sentences=3 if effective_proficiency == "Foundations" else 2
                )

        sec = _build_section(heading, body, enrichment, prose_k, effective_proficiency)
        if sec:
            sections.append(sec)
    ...
    }.get(effective_proficiency, "")

    header = (
        f"# AuraGraph Study Notes\n\n"
        f"**Study Mode: {effective_proficiency}** — {cfg['label']}\n\n"
        f"> {level_banner}\n"
    )
    result = header + "\n\n---\n\n" + "\n\n---\n\n".join(sections)
    return _apply_user_style_overrides(result, custom_note_instruction)
