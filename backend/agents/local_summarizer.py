...
def _is_math_line(line: str) -> bool:
    ...
    if re.search(r'\\[a-zA-Z]+|\$', l):
        return True
    # Comparison/inequality with Greek names (e.g. "alpha + beta >= gamma", "mu <= sigma")
    if _GREEK_RAW.search(l) and re.search(r'[=<>\+\-\*/\^]|>=|<=|!=', l):
        return True
    # Raw greek / math keyword in a short line without sentence structure
    if _GREEK_RAW.search(l) and len(l.split()) < 20 and not re.search(r'\b(is|are|the|denotes|means|where|which|such)\b', l, re.I):
        return True
