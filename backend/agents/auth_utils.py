...
def _normalize_profile(profile: Optional[dict]) -> dict:
    ...
    for key, limit in _PROFILE_LIMITS.items():
        val = src.get(key, "")
        if val is None:
            val = ""
        text = str(val).strip()
...
def get_user_profile(user_id: str) -> dict:
    ...
    try:
        parsed = json.loads(row["profile_json"] or "{}")
...
def update_user_profile(user_id: str, profile: dict) -> dict:
    import json
...
def profile_to_student_context(profile: Optional[dict], proficiency: str = "") -> str:
    p = _normalize_profile(profile)
    parts = []
    if p["class_level"]:
        parts.append(f"Class/Level: {p['class_level']}")
    if p["education_stage"]:
        parts.append(f"Education Stage: {p['education_stage']}")
    if p["institution"]:
        parts.append(f"Institute: {p['institution']}")
    if p["board_university"]:
        parts.append(f"Board/University: {p['board_university']}")
    if p["stream_major"]:
        parts.append(f"Stream/Major: {p['stream_major']}")
    if p["target_exam"]:
        parts.append(f"Target Exam: {p['target_exam']}")
    if p["learning_goal"]:
        parts.append(f"Learning Goal: {p['learning_goal']}")
    if proficiency:
        parts.append(f"Requested Proficiency: {str(proficiency).strip()[:40]}")
    if p["class_level"] or p["education_stage"]:
        stage = " / ".join([x for x in [p["class_level"], p["education_stage"]] if x])
        parts.append(f"Calibrate examples and language for: {stage}")
    return "\n".join(parts)
