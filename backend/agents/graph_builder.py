...
- Edges flow from EARLIER/simpler topics to LATER/more advanced topics.
- Follow the natural lecture progression: introductory concepts point to
  the topics that build upon them.
- Example: "Linear Algebra" -> "Machine Learning" (not the reverse).

Other rules:
- Node "id" must be lowercase with underscores (no spaces).
- Node "label" is the human-readable concept name.
- Every node from the input MUST appear in the output nodes list.
...
- Create a RICH graph — most concepts should have 2+ connections, not just a simple chain.
...
def _build_user_prompt(concepts: list[dict[str, str]]) -> str:
    ...
    return ("Concepts from lecture notes (listed in lecture order, earliest first):\n\n"
            + "\n".join(lines))
