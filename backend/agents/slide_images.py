...
def save_images(notebook_id: str, images: list[ExtractedImage],
    ...
    for img in images:
        try:
            raw_ext = (img.mime or "image/png").split("/")[-1].lower().replace("jpeg", "jpg")
            raw_ext = raw_ext.split(";")[0].strip()
            ext = re.sub(r"[^a-z0-9]", "", raw_ext) or "png"
            path = os.path.join(folder, f"{img.img_id}.{ext}")
            with open(path, "wb") as f:
                f.write(img.data)
            saved_count += 1
        except Exception as e:
            logger.warning("slide_images: failed to save %s for notebook %s: %s", getattr(img, "img_id", "unknown"), notebook_id, e)
    logger.info("slide_images: saved %d/%d images for notebook %s (clear=%s)",
                saved_count, len(images), notebook_id, clear_existing)
