...
export function useFuse(id, deps = {}) {
    ...
    const [customNoteInstruction, setCustomNoteInstruction] = useState('');
    ...
    const handleFuse = useCallback(async () => {
        ...
        try {
            ...
            if (customNoteInstruction.trim()) {
                form.append('custom_note_instruction', customNoteInstruction.trim());
            }
        ...
        setFuseProgress('');
        setVerifyingStep(null);
    }, [id, prof, slidesFiles, textbookFiles, notesFiles, mediaChunks, customNoteInstruction, setNote, setCurrentPage, setMutatedPages, saveNote, extractAndSaveGraph]);
