 ...
 */
export function usePagination(note, { mutating = false, setMutating, setShowSearch, setShowShortcuts, setShowQuizCenter } = {}) {
    ...
    useEffect(() => {
        ...
        const scrollByViewport = (dir) => {
            ...
            if (!scroller) return;
            const delta = Math.max(140, Math.floor(scroller.clientHeight * 0.72));
            scroller.scrollBy({ top: dir * delta, behavior: 'smooth' });
        };

        const goPrevPage = () => {
            setCurrentPage((p) => {
                if (viewMode === 'two') return Math.max(0, p - 2);
        ...
        const goNextPage = () => {
            setCurrentPage((p) => {
                if (viewMode === 'two') {
                    const maxLeft = Math.floor((pages.length - 1) / 2) * 2;
                    return Math.min(maxLeft, p + 2);
                }
        ...
        const h = (e) => {
            ...
            if (mutating) return;
            const isQKey = e.code === 'KeyQ' || (typeof e.key === 'string' && e.key.toLowerCase() === 'q');
            const wantsQuizShortcut = (e.ctrlKey || e.metaKey) && e.shiftKey && isQKey;
            if (e.key === 'ArrowRight') {
                e.preventDefault();
                goNextPage();
                ...
                e.preventDefault();
                setShowSearch?.(true);
            } else if (wantsQuizShortcut) {
                e.preventDefault();
                if (pages.length > 0) setShowQuizCenter?.(true);
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [pages.length, mutating, viewMode]);
