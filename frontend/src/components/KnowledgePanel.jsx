...
export function QuestionCards({ questions, level, onAllAssessed, darkMode = null }) {
    ...
    const lc = {
        bg: isDark ? 'rgba(124, 58, 237, 0.18)' : 'rgba(124, 58, 237, 0.10)',
        border: isDark ? 'rgba(167, 139, 250, 0.45)' : 'rgba(124, 58, 237, 0.30)',
        accent: 'var(--ag-purple)',
                ...
                return (
                    <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                        <div style={{ padding: '10px 12px', fontSize: 12.5, lineHeight: 1.65, color: 'var(--text)' }}>
                            ...
                            <ReactMarkdown remarkPlugins={[remarkMath, remarkGfm]} rehypePlugins={[[rehypeKatex, { throwOnError: false, strict: false }]]} components={{ p: ({ children }) => <span>{children}</span> }}>{normalizeMathMarkdown(q.question || '')}</ReactMarkdown>
                        </div>
                                ...
                                return (
                                    <div key={opt} style={{ display: 'flex', gap: 7, padding: '5px 9px', borderRadius: 7, background: isCorrect ? lc.bg : 'transparent', border: `1px solid ${isCorrect ? lc.border : 'transparent'}`, fontSize: 12, lineHeight: 1.5, transition: 'background 0.2s' }}>
                                        ...
                                        <span style={{ color: isCorrect ? lc.text : 'var(--text2)', flex: 1 }}>
                                            <ReactMarkdown remarkPlugins={[remarkMath, remarkGfm]} rehypePlugins={[[rehypeKatex, { throwOnError: false, strict: false }]]} components={{ p: ({ children }) => <span>{children}</span> }}>{normalizeMathMarkdown(q.options?.[opt] || '')}</ReactMarkdown>
                                        </span>
                        ...
                        <div style={{ borderTop: '1px solid var(--border)' }}>
                            {!isRev ? (
                                ...
                                <div style={{ padding: '10px 12px', background: lc.bg, borderTop: `1px solid ${lc.border}` }}>
                                    ...
                                    <div style={{ fontSize: 11, lineHeight: 1.65, color: lc.text }}>
                                        ...
                                        >
                                            {normalizeMathMarkdown(q.explanation || '')}
                                        </ReactMarkdown>
                                    </div>
                                </div>
                            )}
        ...
        <div style={{ background: 'var(--bg)', borderRadius: 12, border: '1px solid var(--border)', boxShadow: 'var(--shadow-md)', margin: '0 10px 10px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '10px 14px', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                    <div style={{ fontSize: 10, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>🎯 Practice Questions</div>
                    <div style={{ fontSize: 9, color: 'var(--text3)', fontWeight: 600 }}>Tap A Difficulty To Practice</div>
                </div>
...
export default function KnowledgePanel({ nodes, edges, notebookId, onNodeStatusChange, onJumpToSection, onQuizCompleted, darkMode = null, currentPageContent = '' }) {
    ...
    // ── Fixed graph section height (vertical drag resize removed) ───────────
    const graphH = 220;
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    {nodes.length > 0 && (
                        <span style={{ fontSize: 9, color: 'var(--text3)', fontWeight: 500 }}>
                            Click A Node To Practice · Drag Graph To Pan
                        </span>
                    )}
                </div>
                {nodes.length > 0 && (
