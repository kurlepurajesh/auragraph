    ...
    return (
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal fade-in-scale" onClick={e => e.stopPropagation()}
                style={{ maxWidth: 640, width: '96vw', padding: 0, overflow: 'hidden', borderRadius: 16 }}>
                ...
                <div style={{
                    ...
                    <div style={{ marginTop: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, fontWeight: 600, color: isDark ? '#94A3B8' : '#6B7280', marginBottom: 8, alignItems: 'center' }}>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: badge.color, fontSize: 13, fontWeight: 800 }}>
                                <Zap size={16} strokeWidth={1.8} />
                                <span>{data.xp.toLocaleString()} XP</span>
                            </div>
                            {nextBadge && <span style={{ fontSize: 12 }}>{nextBadge.minXp.toLocaleString()} XP — {nextBadge.name}</span>}
                        </div>
...
function StatsTab({ data, badge, darkMode = null }) {
    const isDark = darkMode ?? (typeof document !== 'undefined' && document.documentElement.getAttribute('data-theme') === 'dark');
    const xpPurple = isDark ? '#A78BFA' : '#8B5CF6';
    const xpIndigo = isDark ? '#A5B4FC' : '#6366F1';
    const xpOrange = isDark ? '#FDBA74' : '#FB923C';
    ...
    const stats = [
        { label: 'Total Aura XP', value: data.xp.toLocaleString(), Icon: Zap, color: xpPurple, iconBg: isDark ? 'rgba(167,139,250,0.18)' : 'rgba(139,92,246,0.10)' },
        { label: 'Quizzes Completed', value: data.quizzesCompleted || 0, Icon: FileText, color: xpIndigo, iconBg: isDark ? 'rgba(165,180,252,0.18)' : 'rgba(99,102,241,0.10)' },
        { label: 'Correct Answers', value: data.correctAnswers || 0, Icon: CheckCircle2, color: xpOrange, iconBg: isDark ? 'rgba(253,186,116,0.18)' : 'rgba(251,146,60,0.10)' },
        { label: 'Accuracy', value: data.totalAnswers > 0 ? `${Math.round((data.correctAnswers / data.totalAnswers) * 100)}%` : '—', Icon: Target, color: xpIndigo, iconBg: isDark ? 'rgba(165,180,252,0.18)' : 'rgba(99,102,241,0.10)' },
        { label: 'Doubts Resolved', value: data.doubtsAsked || 0, Icon: Lightbulb, color: xpPurple, iconBg: isDark ? 'rgba(167,139,250,0.18)' : 'rgba(139,92,246,0.10)' },
        { label: 'Highlights Made', value: data.highlightsAdded || 0, Icon: PencilLine, color: xpOrange, iconBg: isDark ? 'rgba(253,186,116,0.18)' : 'rgba(251,146,60,0.10)' },
    ];
    return (
        <>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {stats.map(s => (
                <div key={s.label} style={{ background: 'var(--surface)', borderRadius: 16, padding: '14px 16px', border: '1px solid var(--ag-dashboard-card-border)', boxShadow: 'var(--ag-dashboard-card-shadow)', transition: 'all 0.18s' }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: s.iconBg, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8, border: '1px solid var(--ag-dashboard-card-border)' }}>
                        <s.Icon size={22} strokeWidth={1.8} color={s.color} />
                    </div>
        ...
        <div style={{ marginTop: 16, padding: '14px 16px', background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--ag-dashboard-card-border)', boxShadow: 'var(--ag-dashboard-card-shadow)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, background: badge.bg, color: badge.color, border: `1px solid ${badge.border}`, borderRadius: 14, padding: '4px 10px', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                        {profile.learning_style === 'conceptual' ? <Brain size={14} strokeWidth={1.8} color={badge.color} /> :
                         profile.learning_style === 'visual'     ? <Eye size={14} strokeWidth={1.8} color={badge.color} /> :
                         profile.learning_style === 'practice'   ? <TrendingUp size={14} strokeWidth={1.8} color={badge.color} /> :
                                                                    <Scale size={14} strokeWidth={1.8} color={badge.color} />}
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text3)', borderRadius: 12, padding: '4px 10px', background: 'var(--bg)', border: '1px solid var(--border)' }}>
                        Preferred: {profile.preferred_proficiency}
                    </span>
                </div>
                {profile.weak_concepts?.length > 0 && (
                    <div style={{ marginBottom: 6 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: isDark ? '#F87171' : '#EF4444', marginBottom: 4, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                            <AlertCircle size={14} strokeWidth={1.8} /> Needs work
                        </div>
                ...
                {profile.strong_concepts?.length > 0 && (
                    <div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: isDark ? '#86EFAC' : '#10B981', marginBottom: 4, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                            <ShieldCheck size={14} strokeWidth={1.8} /> Strong areas
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
