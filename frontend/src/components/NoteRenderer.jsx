...
export default function NoteRenderer({ content, onDoubtLink, fontSize = 16, darkMode = null }) {
    ...
    const mk = {
        h1({ children }) {
            return (
                <div style={{ marginBottom: 32, paddingBottom: 20, borderBottom: '2px solid var(--ag-purple-border)' }}>
        ...
        code({ className, children }) {
            if (!className) return <code style={{ background: 'var(--ag-purple-bg)', color: 'var(--ag-purple-medium)', borderRadius: 5, padding: '2px 7px', fontSize: 13, fontFamily: '"JetBrains Mono","Courier New",monospace', fontWeight: 600, border: '1px solid var(--ag-purple-border)' }}>{children}</code>;
            return (
        ...
        blockquote({ children }) {
            ...
            if (isIntuition) return (
                <div className="nr-keep-together nr-callout" style={{ background: isDark ? '#1E1B4B' : 'var(--ag-purple-bg)', border: `1px solid ${isDark ? '#4C1D95' : 'var(--ag-purple-border)'}`, borderLeft: '3px solid var(--ag-purple)', borderRadius: 8, padding: '12px 16px', margin: '16px 0' }}>
                    ...
                    <div style={{ fontSize: 13.5, color: isDark ? '#DDD6FE' : 'var(--ag-purple-deep)', lineHeight: 1.7, fontFamily: '"DM Sans",sans-serif' }}>{children}</div>
                </div>
            );
            if (isWarning) return (
        ...
        hr() { return <div style={{ border: 'none', height: 1, background: 'linear-gradient(90deg,transparent,var(--ag-purple-border) 30%,var(--ag-purple-border) 70%,transparent)', margin: '32px 0' }} />; },
        ...
        table({ children }) {
            return (
                <div className="nr-keep-together" style={{ overflowX: 'auto', margin: '18px 0', borderRadius: 10, border: isDark ? '1px solid #475569' : '1px solid var(--ag-purple-border)', boxShadow: isDark ? '0 4px 16px rgba(2,6,23,0.45)' : '0 2px 8px rgba(var(--ag-purple-rgb),0.06)' }}>
                    <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13.5, fontFamily: '"DM Sans",sans-serif' }}>{children}</table>
                </div>
            );
        },
        thead({ children }) { return <thead style={{ background: isDark ? 'linear-gradient(135deg,#1e293b,#0f172a)' : 'linear-gradient(135deg,var(--ag-purple-border),var(--ag-purple-bg))' }}>{children}</thead>; },
        tbody({ children }) { return <tbody>{children}</tbody>; },
        tr({ children, node }) {
            const idx = node?.position?.start?.line ?? 0;
            return <tr style={{ borderBottom: isDark ? '1px solid #334155' : '1px solid var(--ag-purple-border)', background: isDark ? (idx % 2 === 0 ? '#0f172a' : '#111827') : (idx % 2 === 0 ? '#FAFAFA' : '#FFFFFF') }}>{children}</tr>;
        },
        th({ children }) { return <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 700, color: isDark ? '#C4B5FD' : 'var(--ag-purple-deep)', borderBottom: isDark ? '2px solid #6366F1' : '2px solid var(--ag-purple-ring)', whiteSpace: 'nowrap', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{children}</th>; },
