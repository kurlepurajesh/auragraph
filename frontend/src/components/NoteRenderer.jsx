...
export default function NoteRenderer({ content, onDoubtLink, fontSize = 16 }) {
    const mk = {
        ...
        h4({ children }) {
            // Exercise / Example / worked-problem headings generated by the LLM
            const text = typeof children === 'string' ? children : '';
            const isExercise = /^(exercise|example|problem|worked)/i.test(text);
            if (isExercise) return (
                <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderLeft: '3px solid #3B82F6', borderRadius: 8, padding: '10px 14px', margin: '14px 0' }}>
            ...
            return <div style={{ marginTop: 18, marginBottom: 8, fontSize: 13, fontWeight: 600, color: '#374151', fontFamily: '"DM Sans",sans-serif' }}>{children}</div>;
        },
        ...
        blockquote({ children }) {
            ...
            if (isExamTip) return (
                <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '12px 14px', margin: '16px 0', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 16, flexShrink: 0 }}>🎯</span>
                    <div style={{ fontSize: 13.5, color: '#78350F', lineHeight: 1.6, fontFamily: '"DM Sans",sans-serif' }}>{children}</div>
                </div>
            );
            if (isFormula) return (
                <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderLeft: '3px solid var(--ag-purple)', borderRadius: 8, padding: '12px 16px', margin: '16px 0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}><span style={{ fontSize: 14 }}>🔢</span><span style={{ fontSize: 11, fontWeight: 700, color: '#475569', textTransform: 'uppercase' }}>Formula</span></div>
                    <div style={{ fontSize: 13.5, color: '#1E293B', lineHeight: 1.7, fontFamily: '"DM Sans",sans-serif' }}>{children}</div>
                </div>
            );
            if (isIntuition) return (
                <div style={{ background: '#F5F3FF', border: '1px solid #EDE9FE', borderLeft: '3px solid var(--ag-purple)', borderRadius: 8, padding: '12px 16px', margin: '16px 0' }}>
                    ...
                    <div style={{ fontSize: 13.5, color: '#1E0B3D', lineHeight: 1.7, fontFamily: '"DM Sans",sans-serif' }}>{children}</div>
                </div>
            );
            if (isWarning) return (
                <div style={{ background: '#FEF2F2', border: '1px solid #FEE2E2', borderRadius: 8, padding: '12px 14px', margin: '12px 0', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 16, flexShrink: 0 }}>⚠️</span>
                    <div style={{ fontSize: 13, color: '#991B1B', lineHeight: 1.6, fontFamily: '"DM Sans",sans-serif' }}>{children}</div>
                </div>
            );
            if (isMutation) return (
                <div style={{ background: '#F0FDF4', border: '1px solid #DCFCE7', borderRadius: 8, padding: '12px 14px', margin: '12px 0', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 16, flexShrink: 0 }}>✨</span>
        ...
        p({ children }) { return <p style={{ marginBottom: 14, lineHeight: 'inherit', color: '#1C1917', fontFamily: 'inherit', fontSize, letterSpacing: '0.005em' }}>{children}</p>; },
    ...
    return (
        <div style={{ color: '#1C1917', fontFamily: '"Source Serif 4",Georgia,serif', lineHeight: 2.05 }}>
            <ReactMarkdown
