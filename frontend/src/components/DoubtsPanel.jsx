...
const IM = ({ text, darkMode = null }) => {
    ...
    <ReactMarkdown
        ...
        components={{
            p:     ({ children }) => <span style={{ display: 'block', marginBottom: 8, lineHeight: 1.7 }}>{children}</span>,
            ol:    ({ children }) => <ol style={{ margin: '6px 0 10px', paddingLeft: 18, listStylePosition: 'inside' }}>{children}</ol>,
            ul:    ({ children }) => <ul style={{ margin: '6px 0 10px', paddingLeft: 18, listStylePosition: 'inside' }}>{children}</ul>,
                    ...
                    return (
                        <div key={d.id} id={'doubt-' + d.id}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, marginBottom: 8 }}>
                                <div style={{ maxWidth: '84%', background: 'var(--ag-purple)', color: '#fff', borderRadius: '14px 14px 6px 14px', padding: '12px 18px', fontSize: 13, lineHeight: 1.6 }}>{d.doubt}</div>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'flex-start', paddingLeft: 8 }}>
                                {d.success ? (
                                    <div style={{ maxWidth: '90%', background: isDark ? '#1E1B4B' : 'var(--ag-purple-bg)', border: isDark ? '1px solid #4C1D95' : '1px solid #DDD6FE', borderRadius: '8px 12px 12px 12px', padding: '12px 16px', fontSize: 13, lineHeight: 1.75, color: isDark ? '#E9D5FF' : '#3B0764' }}>
                                        <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--ag-purple)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}><GitBranch size={12} /> AuraGraph</div>
                                        <div style={{ color: isDark ? '#DDD6FE' : 'var(--ag-purple-deep)', paddingLeft: 2 }}><IM text={preview} darkMode={isDark} /></div>
