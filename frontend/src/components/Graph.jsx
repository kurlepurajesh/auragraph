...
export function KnowledgeGraph({ nodes, edges, onNodeClick, selectedNodeId, highlightNodeIds = [] }) {
    ...
    const dragRef = React.useRef({ dragging: false, startX: 0, startY: 0, startViewX: 0, startViewY: 0, moved: false });
    const [viewBoxOrigin, setViewBoxOrigin] = React.useState({ x: 0, y: 0 });
    const [isPanning, setIsPanning] = React.useState(false);

    const getPos = React.useCallback((n) => ({ cx: (n.x / 100) * (W - 60) + 30, cy: (n.y / 100) * (H - 60) + 30 }), [W, H]);

    const panBounds = React.useMemo(() => {
        if (!nodes?.length) return { minX: 0, maxX: 0, minY: 0, maxY: 0 };
        const PAD_X = 44; // include label room
        const PAD_Y_TOP = 26;
        const PAD_Y_BOTTOM = 34; // include text label under nodes
        const xs = nodes.map(n => getPos(n).cx);
        const ys = nodes.map(n => getPos(n).cy);
        const contentLeft = Math.min(...xs) - PAD_X;
        const contentRight = Math.max(...xs) + PAD_X;
        const contentTop = Math.min(...ys) - PAD_Y_TOP;
        const contentBottom = Math.max(...ys) + PAD_Y_BOTTOM;

        const minX = Math.min(0, contentRight - W);
        const maxX = Math.max(0, contentLeft);
        const minY = Math.min(0, contentBottom - H);
        const maxY = Math.max(0, contentTop);
        return { minX, maxX, minY, maxY };
    }, [nodes, getPos]);

    const clampOrigin = React.useCallback((origin) => ({
        x: Math.max(panBounds.minX, Math.min(origin.x, panBounds.maxX)),
        y: Math.max(panBounds.minY, Math.min(origin.y, panBounds.maxY)),
    }), [panBounds]);

    React.useEffect(() => {
        setViewBoxOrigin(prev => clampOrigin(prev));
    }, [clampOrigin]);
    ...
    React.useEffect(() => {
        const onMove = (ev) => {
            ...
            const dx = (point.clientX - dragRef.current.startX) * scaleX;
            const dy = (point.clientY - dragRef.current.startY) * scaleY;
    ...
    const startPan = (ev) => {
        ...
        dragRef.current = {
            ...
            startViewX: viewBoxOrigin.x,
            startViewY: viewBoxOrigin.y,
            moved: false,
        };
        setIsPanning(true);
    };
    ...
    const getPosLocal = n => ({ cx: (n.x / 100) * (W - 60) + 30, cy: (n.y / 100) * (H - 60) + 30 });
    const nodeById = Object.fromEntries(nodes.map(n => [n.id, n]));
    return (
        <svg
            ref={svgRef}
            ...
            viewBox={`${viewBoxOrigin.x} ${viewBoxOrigin.y} ${W} ${H}`}
        ...
        >
            ...
            {edges.map((e, i) => {
                const s = nodeById[e[0]], d = nodeById[e[1]];
                if (!s || !d) return null;
                const sp = getPosLocal(s), dp = getPosLocal(d);
