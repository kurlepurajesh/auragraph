...
export default function SourceInputPanel({
    ...
    React.useEffect(() => {
        if (!forcedTab) return;
        if (SOURCES.some(s => s.id === forcedTab)) {
    ...
    return (
        <div>
            {/* Tab bar */}
            <div data-tour="upload-source-tabs" style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--surface)', padding: 4, borderRadius: 12, border: '1px solid var(--border)' }}>
                {SOURCES.map(s => {
                    const Icon = s.icon;
                    ...
                    return (
                        <button key={s.id} data-tour={`upload-tab-${s.id}`} onClick={() => setActiveTab(s.id)}
                            style={{
            ...
            <div data-tour="upload-custom-prompt" data-tour-pad="10" style={{ marginTop: 14, padding: '12px 12px 10px', borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text2)' }}>How should AuraGraph write your notes?</div>
                    {!!customNoteInstruction?.trim() && (
                        <button
                            onClick={() => setCustomNoteInstruction('')}
                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: 11, textDecoration: 'underline', padding: 0 }}
                        >
                            Clear
                        </button>
                    )}
                ...
                <textarea
                    value={customNoteInstruction || ''}
                    onChange={e => setCustomNoteInstruction(e.target.value.slice(0, 500))}
                    placeholder="Examples: focus on numerical problems and solved examples; give point-wise revision notes; keep explanations detailed with intuition"
                    style={{
                        width: '100%',
                        minHeight: 72,
                        resize: 'vertical',
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                    {[
                        'Focus on problems + solved examples',
                        'Point-wise notes for quick revision',
                        'Detailed explanation with intuition',
                        'Exam-oriented: formulas + common mistakes',
                    ].map((preset) => (
                        <button
                            key={preset}
                            onClick={() => setCustomNoteInstruction(preset)}
                            style={{
                                border: '1px solid var(--border)',
                                background: 'var(--bg)',
                                color: 'var(--text3)',
