...
import { X, Loader2, FileText, Trophy, TrendingUp, Dumbbell, AlertTriangle, CheckCircle2, XCircle, Zap, Target } from 'lucide-react';
import { API, authHeaders } from './utils';

export default function GeneralExamModal({ nodes, notebookId, onClose, onQuizCompleted, onAttemptSaved = null }) {
    ...
    const generalFeedback = {
        good: {
            headline: 'You rocked it!',
            message: 'Excellent command across mixed concepts. Keep this momentum and challenge yourself again.',
        },
        mid: {
            headline: 'Nice progress!',
            message: 'You are improving well. Revisit the missed concepts and retake the test for a stronger score.',
        },
        bad: {
            headline: 'Let\'s build this up together.',
            message: 'Start with Quick Review and concept-wise revision, then retry the General Test.',
        },
    }[performanceBand];
    ...
    React.useEffect(() => {
        ...
        const conceptsTested = (questions || []).map(q => ({
            label: q?.concept || 'Concept',
            status: 'all',
        }));
        ...
        fetch(`${API}/api/notebooks/${notebookId}/quizzes/complete`, {
            ...
            .catch(() => { });
    }, [done, loading, total, notebookId, questions, responses, score, onAttemptSaved]);
            ...
            <div className="modal fade-in-scale" onClick={e => e.stopPropagation()} style={{ maxWidth: 560, width: '96vw', maxHeight: '90vh', overflowY: 'auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <FileText size={22} color="var(--ag-purple)" />
                        <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--ag-purple)', fontWeight: 700, marginBottom: 8, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {pct >= 80 && <Target size={13} />}
                        ...
                        <div style={{ fontSize: 14, color: 'var(--text3)', marginBottom: 20 }}>
                            <div style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{generalFeedback.headline}</div>
                            <div>{generalFeedback.message}</div>
                        </div>
                    ...
                    <>
                        ...
                        {current.concept && (
                            <div style={{ marginBottom: 10, display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--purple-light)', color: 'var(--purple)', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 600 }}>
                                <FileText size={13} /> {current.concept}
                            </div>
                        )}
