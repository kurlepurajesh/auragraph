...
export function StudyTimer() {
    ...
    return (
        <div ref={triggerRef} style={{ position: 'relative', flexShrink: 0 }}>
            {!open && (
                <button className="ag-toolbar-icon-btn" onClick={() => setOpen(true)} title="Pomodoro timer"
...
export function ShortcutsModal({ onClose }) {
    React.useEffect(() => {
        ...
        // Capture phase prevents Escape from leaking to background listeners.
        window.addEventListener('keydown', onKeyDown, true);
        return () => window.removeEventListener('keydown', onKeyDown, true);
    }, [onClose]);

    const shorts = [
        ['Up / Down / PgUp / PgDn', 'Scroll in note'],
        ['Left / Right', 'Previous / Next page'],
        ['Click page counter', 'Jump to any page number'],
        ['Ctrl+Shift+Q', 'Open Quiz Center'],
        ['Ctrl+D', 'Ask a doubt / Rewrite page'],
        ['Ctrl+F', 'Search in notes'],
        ['Quiz: 1 / 2 / 3 / 4', 'Select option A/B/C/D'],
        ['Quiz: Enter', 'Next question'],
        ['Ctrl+Enter', 'Submit doubt in modal'],
        ['Esc', 'Close modal / selection'],
    ];
    return (
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal fade-in-scale" onClick={e => e.stopPropagation()} style={{ maxWidth: 500, width: '96vw' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {shorts.map(([k, d]) => (
                        <div key={k} style={{ display: 'grid', gridTemplateColumns: '220px 1fr', alignItems: 'center', gap: 12, padding: '8px 12px', borderRadius: 8, background: 'var(--surface)', border: '1px solid var(--border)' }}>
                            <kbd style={{ whiteSpace: 'nowrap', width: 'fit-content' }}>{k}</kbd>
                            <span style={{ fontSize: 12, color: 'var(--text2)', textAlign: 'left' }}>{d}</span>
                        </div>
                    ))}
                </div>
