...
function ensureUserStreakAfterAuth(user, mode) {
    const key = getStreakKey(user?.id);
    const hasUserScopedStreak = !!localStorage.getItem(key);
    ...
    if (hasUserScopedStreak) return;

    const legacy = localStorage.getItem('ag_streak');
    if (legacy) {
...
export default function LoginPage() {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            ensureUserStreakAfterAuth(data, mode);
            navigate('/dashboard');
        } catch {
            ...
            localStorage.setItem('ag_token', 'demo-token');
            localStorage.setItem('ag_user', JSON.stringify(mockUser));
            ensureUserStreakAfterAuth(mockUser, mode);
            navigate('/dashboard');
        }
        setLoading(false);
    };
