...
export default function AuraGraph() {
    ...
    const handleMutate = useCallback(async (page, doubt) => {
        try {
            if (!notebookId) throw new Error('Notebook ID missing for mutation request.');
            ...
            if (!res.ok) {
                let detail = `Mutation failed (${res.status})`;
                try {
                    const err = await res.json();
            ...
            console.warn('AuraGraph mutate failed:', e?.message || e);
    ...
    }, [notebookId, currentPage, note, dispatch]);
